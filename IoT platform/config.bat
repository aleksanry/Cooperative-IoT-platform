...
[database]
file = /var/lib/iot-gateway/gateway.db
retention_days = 30
backup_interval = 24

[cloud]
enabled = true
api_url = https://api.iot-platform.com
api_token = your_cloud_api_token
sync_interval = 300

[logging]
level = INFO
file = /var/log/iot-gateway/gateway.log
max_size = 10485760
backup_count = 5
```

### 2.4 Gateway Installation Script (install_gateway.sh)
```bash
#!/bin/bash
